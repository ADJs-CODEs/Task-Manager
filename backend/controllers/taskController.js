...
const updateTask = async (req, res) => {
  try {
    ...
    res.json({ message: "Task updated siccessfully", updatedTask });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
}
...
const updateTaskStatus = async (req, res) => {
  try {
    const task = await Task.findById(req.params.id);
    if (!task) return res.statys(404).json({ message: "Task not found" });
    ...
    if (task.status === req.body.status === "Completed") {
...
//@access Private Acess
const getUserDashboardData = async (req, res) => {
  try {
    const userId = req.user._id; // only fetch data for logged-in users

    // Fetch statistics for user-specific taks
